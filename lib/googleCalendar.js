...
export async function findCalendarEvents({ startIso = "", query = "", maxResults = 10 } = {}) {
  const now = new Date();
  const exactStart = startIso ? new Date(startIso) : null;
  const timeMin = exactStart && !Number.isNaN(exactStart.getTime())
    ? new Date(exactStart.getTime() - 5 * 60_000)
    : now;
  const timeMax = exactStart && !Number.isNaN(exactStart.getTime())
    ? new Date(exactStart.getTime() + 5 * 60_000)
...
export async function deleteCalendarEvent(eventId) {
  if (!eventId) throw new Error("削除する予定を特定できませんでした。");
  await googleRequest(
