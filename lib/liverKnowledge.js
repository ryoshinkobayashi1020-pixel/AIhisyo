...
// この質疑応答はクライアントへ提示する回答を作るために使う。
// 事務所側の取り分（事務所報酬・バディ率・代理店ロイヤリティ・紹介料など）は
// 社内および代理店向けの情報なので、検索対象から外して外部AIにも渡さない。
//
// 第1層: 収益条件そのものを扱うフォルダを丸ごと除外する
...
// 第2層: 残った資料の中にも事務所側の収益の話が混ざるため、そのページだけ落とす。
// 「クリエイター報酬」「ライバー報酬」はライバー本人が受け取る分なので除外しない。
...
function isExcludedDocument(doc) {
  const label = `${doc.category || ""} / ${doc.file_name || ""}`;
  return EXCLUDED_CATEGORY_PATTERNS.some(pattern => pattern.test(label));
}
...
async function loadCorpus(includeRevenue = false) {
  const cacheKey = includeRevenue ? "full" : "client";
  if (corpusCache?.key === cacheKey && Date.now() - corpusCache.loadedAt < CORPUS_TTL_MS) return corpusCache;
  const supabase = getClient();
  if (!supabase) throw new Error("Supabaseが設定されていません。");
  ...
  // クライアント向けの窓口では、事務所側の収益に関する資料・ページをここで完全に取り除く。
  // 検索対象にも回答生成の材料にもならないため、外部AIへ送られることもない。
  // 社内・代理店向けの窓口では報酬資料も必要なので、そのまま残す。
  const documents = includeRevenue
    ? (allDocuments || [])
    : (allDocuments || []).filter(doc => !isExcludedDocument(doc));
  const allowedIds = new Set(documents.map(doc => doc.id));

  // バディ制度は事務所報酬を投げ銭として還元する任意の仕組みで、
  // 事務所報酬そのものの計算式ではない。AIがこの資料の「バディ率30％」を
  // 計算根拠に使って過大な金額を答える事故が続いたため、検索対象から外す。
  const buddyDocumentIds = new Set(
    documents.filter(doc => /バディ制度/.test(`${doc.category} ${doc.file_name}`)).map(doc => doc.id)
  );

  const prepared = [];
  for (const chunk of chunks) {
    if (!allowedIds.has(chunk.document_id)) continue;
    if (buddyDocumentIds.has(chunk.document_id)) continue;
    const text = stripBoilerplate(chunk.content);
    if (!includeRevenue && isExcludedContent(text)) continue;
    if (/バディ(率|報酬)/.test(text)) continue;
    prepared.push({ ...chunk, text, gramSet: new Set(bigrams(text)) });
  }
...
export async function searchKnowledge(query, limit = 8, { includeRevenue = false } = {}) {
  const normalized = normalizeQuery(query);
  const queryGrams = [...new Set(bigrams(normalized || query))];
  if (!queryGrams.length) return [];

  const corpus = await loadCorpus(includeRevenue);
