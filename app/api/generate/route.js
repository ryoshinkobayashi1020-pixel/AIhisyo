...
const MANA_SHIN_YUKI_QUALITY_GATE = `【しん・ゆうき共通・日本語と説明の最終品質確認】
完成台本を出力する前に、次を一文ずつ内部確認する。一つでも満たさない場合は、部分修正ではなく前後を含めて書き直す。確認結果は出力しない。
- 冒頭の主張または質問が一度で理解でき、その後の全発言が同じ論点を深めている。
- 結論だけでなく「なぜそう言えるのか」「現場では何をするのか」「その結果どう変わるのか」まで説明している。
- 質問には最初の一文で直接答え、その後に理由と具体例を続ける。質問を言い換えただけの回答や、理念だけの回答にしない。
- 反論や疑問を出した場合は、次の回答で必ず解消する。説明しないまま別の話題や標語へ移らない。
- 主語と対象を明示する。「人」「周り」「みんな」「それ」の範囲が曖昧なら、「後輩スタッフ」「同じ店舗のスタッフ」「お客様」などへ具体化する。
- 時系列、原因、行動、結果の順序を守る。気づく前に判断する、理由を示す前に結論へ飛ぶなどの前後逆転を起こさない。
- 一文に目的、評価、理由を詰め込みすぎない。長い文は二文に分け、助詞、動詞、主語と述語の対応を確認する。
- 「店をつくる」「人を大切にする」「信頼される」などの抽象語を使ったら、美容室で実際に取る行動を直後に一つ示す。
- 実在スタッフの体験として確認できない内容は断定しない。結婚、家賃、生活費などは想定される悩みとして自然に扱い、未確認の個人情報を作らない。
- 声に出して読み、日常の日本語として引っかかる言葉、意味を二度考えないと分からない文、質問と噛み合わない返答を残さない。

今回の修正から学習する禁止例と修正方針:
- 「そこまで悩むまで気づけなかった」のような同語反復を避け、「辞めたいと思うほど悩んでいることに気づけなかった」のように関係を明確にする。
- 「家賃や生活費を払った先」のような不自然な表現を避け、「家賃や生活費を払ったあと、手元にいくら残るのか」のように生活の順序で話す。
- 「教える時間は取れなくないですか」のような二重否定を避け、「教える時間を取るのは難しくないですか」と自然に聞く。
- 「後輩の失敗を見る」「スタッフ同士の空気を見る」のような意味の広い動詞を避け、「原因を一緒に考える」「空気にも目を配る」のように行動を示す。
- 「仲間の売上が上がった」の主語が必要なら、「後輩の売上が自分を超えた」のように誰の変化かを明示する。
...
しん専用:
- 一つの主張に絞り、主張、過去の考え、問題が起きる理由、具体的な行動、現在の考えの順を基本にする。
- 最後の会社理念は途中の説明から自然に導く。説明不足のまま「人を大切にする会社です」で終わらせない。

ゆうき専用:
- カメラマンの各質問は直前の回答を受けて進める。答えを知っている質問、同じ質問の繰り返し、突然の話題変更をしない。
- 美容師スタッフは現場の実感、瀬川社長は会社がその環境を整える理由を話し、同じ説明を繰り返さない。
- 2人・3人形式のどちらでも、最後は最初の質問への答えとして回収する。

【10本の実地修正から確定した最終ルール】
以下は上の口調・構成ルールより新しく、矛盾する場合はこちらを優先する。

しん:
- 一人語りは、経験、転機、反省、信念を「変わった」「気づいた」「した」のように短く言い切る。ただし「〜なんだ」を機械的に付けない。
- 結論へ飛ばず、「夢は最初は漠然としていてもいい」のように聞き手を受け止める一文を置いてから主張へ進む。
- 「強い」「輝かせる」などの抽象語だけで終えず、「誰が何にふさわしいのか」「どんな立場である必要があるのか」を明示する。
- 確定している会社の特徴は「会社でいたい」ではなく「会社です」と事実として述べる。本人の考えは「〜と思う」と区別する。

ゆうき:
- 対談は、しんの一人語りのような断言調へ統一しない。瀬川社長は「〜だよね」「〜と思う」「〜かな」「〜やね」のように、経験を振り返る自然な会話口調を使ってよい。美容師スタッフは自然な丁寧語で話す。
- 毎回カメラマンの質問から始めない。美容師スタッフの実体験から始め、カメラマンが別の見方を出し、社長が会社の考えを補足する流れも使う。
- カメラマンは「それなら」を機械的に付けない。直前の回答を受けて、動機、実際の行動、働いてみた結果など、次に知りたいことを自然に聞く。
- 反論では、出来高制など別の選択肢にも合う人がいることを一度認める。相手を否定して自社だけを正解にせず、本人が何を優先して選んだかを話す。
- 求人・働き方では、家賃や生活費を払ったあとに手元へいくら残るか、社会保険、結婚や将来、技術を学ぶ時期の収入不安など、生活の実感を入れる。ただし実在スタッフの未確認の経歴として断定しない。
- 美容師スタッフは「不安だったこと→選んだ理由→働いてみて変わったこと」を自然につなぐ。瀬川社長は同じ内容を繰り返さず、会社が環境を整える理由や人を大切にする経営へつなげる。
- 会社の制度は「直接雇用で、毎月の収入が安定しやすい環境を整えています」のように正確に説明する。確認済みの会社の役割は「安心して続けられる環境を提供するのがマナコーポレーションです」と事実として締めてよい。
- 「決めつけていたんだよね」「いいことがなかった」のように、当時の行動、起きた結果、現在の反省を順に話す。一般論の標語だけで終わらせない。
- 「勤務時間で練習時間を儲けている」のような誤変換を残さず、「勤務時間内に練習する時間を設けている」と、助詞、漢字、句読点まで直す。
- 冒頭、話者の順番、質問の型、締め方を毎回変え、同じインタビュー構成へ固定しない。

【実際に確定した修正内容】
次は完成台本へ毎回そのまま転載する例文ではなく、日本語、質問順、説明の深さを判断する基準として使う。
- しんの店長テーマでは、「店舗全体を良くすることはできない」「周りを輝かせる立場である必要がある」「それを本気で喜べる人が店長にふさわしいと思う」「仲間を輝かせられる人を育てる会社です」という明確さを基準にする。
- しんの夢テーマでは、「やるべきことが初めて見えた」と簡潔に言い切り、「夢は最初は漠然としていてもいい」を挟んでから、完璧な計画は必要ないという結論へ進む。
- しんの退職テーマでは、「自分に問いかける必要がある」と振り返り、退職後に外で人や会社と出会い学ぶことを肯定し、「困ったときに連絡を取り合える関係であり続けられる」「経営者の大切な仕事なんだと思う」と関係の継続へ着地する。
- ゆうきの人間関係テーマでは、「これまでどうしていましたか。今考えると一番反省したことは何ですか」と時系列で聞き、「原因はこれだと決めつけていたんだよね」と当時を振り返る。続く質問は、問題から逃げたかったのか、早く解決したかったのかという動機を聞く。
- ゆうきの再入社テーマでは、再び辞める可能性も否定せず、戻りたい理由、以前の悩み、会社が支えられることを確認し、最後は抽象的な願望ではなく、そのご縁を大切に迎え入れるという行動で締める。
- ゆうきのお客様テーマでは、技術だけでなく前回の悩みを覚えて次回来店時に尋ねる行動を入れる。会話が得意でなくても、表情を見る、仕上がりを一緒に確認する、時間や説明など小さな約束を守ることが信頼につながると説明する。
- ゆうきの売上テーマでは、「売上が大切か」ではなく「その売上をどうつくるか」を論点にする。物販等で単価だけを上げる例を出し、短期的なプラスは認めたうえで、再来店やスタッフ同士の協力まで見る。
- ゆうきの育成テーマでは、「何が違うか」ではなく「どういう行動が違うか」と聞く。短い声かけ、勤務時間内の練習時間、後輩の成長が店舗全体の信頼につながる流れを示す。
- ゆうきの生活テーマでは、出来高制サロンにも興味があったことを認めつつ、収入、社会保険、生活の変化への不安を本人の比較として話す。「マナ」ではなく「マナコーポレーション」と正式名称を使う。
- ゆうきの働き方テーマでは、美容師スタッフの「家賃や生活費を払ったあと、手元にいくら残るのか分からない」という実感から始めてよい。「働いてみてどうでしたか」と自然に聞き、技術、なりたい美容師像、長く働く未来を考えやすくなった変化へつなげる。`;
...
${job.narrationReference || job.staffDialogueReference ? `
${MANA_SHIN_YUKI_QUALITY_GATE}` : ""}`;
}
...
- 質問役を出す場合は「スタッフ」ではなく「カメラマン」と表記する。冒頭の発言だけで今回のテーマが分かるようにするが、毎回カメラマンの質問から始める必要はない
- 美容師本人の経験や現場の実感が必要なテーマでは、「カメラマン」「瀬川社長」「美容師スタッフ」の3人形式にする。美容師スタッフの未登録の氏名、勤続年数、役職、実績は作らない
- 3人形式では、美容師スタッフが現場の実感、カメラマンが視聴者の疑問や別の見方、瀬川社長が会社の考え方や背景を担う。テーマに合わせて話し始める人物と順番を変え、人数を増やすだけの発言は入れない
...
- 質問から始める場合は、「新店舗を作るとしたら」「新人教育を任せるとき」など、なぜ今その質問をするのか分かる場面を置く。美容師スタッフの不安や経験から始める場合は、その発言自体で状況が分かればよい。確認できない出来事は仮定にする
...
- カメラマンが一度は、直前の回答を受けた反論または再質問をする。「でも」「それなら」「具体的には」を機械的に付けず、動機、行動、結果、別の選択肢のいずれかを自然に聞く
...
- しんの一人語りの口調へ寄せない。瀬川社長は「〜だよね」「〜と思う」「〜かな」「〜やね」など自然な会話口調を使い、美容師スタッフは自然な丁寧語で話す
- 出来高制など別の働き方を一方的に否定せず、合う人もいることを認めたうえで、本人が何を優先してマナコーポレーションを選んだかを話す
- 毎回同じ話者順、同じ質問、同じ締め方にせず、美容師スタッフの実体験から始める型も含めて構成を変える
- 誤変換、助詞抜け、句読点不足を残さない。「勤務時間で練習時間を儲ける」ではなく「勤務時間内に練習する時間を設ける」のように、意味が一度で伝わる日本語へ直す
