...
function isCalendarInstruction(text) {
  ...
  return /(空いて|空き(?:の日|時間)|可能(?:ですか|でしょうか|な日|な時間)|都合(?:は|の良い)|いつ(?:にします|がいい|が空いて|空いて|なら)|何日(?:が|なら)?.*可能|何時(?:が|なら)?.*可能|日程(?:を|の)?(?:確認|相談|調整)|撮影日(?:を|の)?(?:確認|相談)|(?:この|その)(?:時間|日時|日程|候補)で|(?:1|１|一|2|２|二|3|３|三)(?:つ目|番目|番)で|\d{1,2}月\d{1,2}日.*\d{1,2}時|\d{1,2}時(?:半)?(?:で|から).*(?:お願い|決定|確定))/.test(value)
