...
function explicitStaffNameIndex(normalizedText, staffName) {
  let index = normalizedText.indexOf(staffName);
  while (index >= 0) {
    const tail = normalizedText.slice(index + staffName.length);
    const followedByAnotherStaff = STAFF.some(staff =>
      staff.name !== staffName && tail.startsWith(staff.name)
    );
    const followedByCallingCue = /^(?:さん|ちゃん|くん|君)?(?:と|や|および|ならびに|に|へ|は|が|の|を|で|って|お願い|おねがい|台本|動画|編集|求人|語り|請求|予定|[0-9０-９一二三四五六七八九十]+本|$)/.test(tail);
    if (followedByAnotherStaff || followedByCallingCue) return index;
    index = normalizedText.indexOf(staffName, index + staffName.length);
  }
  return -1;
}

function findExplicitStaffListByReading(normalizedText) {
  let manaNameIndex = explicitStaffNameIndex(normalizedText, "まな");
  while (manaNameIndex >= 0 && normalizedText.slice(manaNameIndex).startsWith("まなこーぽれーしょん")) {
    const nextText = normalizedText.slice(manaNameIndex + "まなこーぽれーしょん".length);
    const nextRelativeIndex = explicitStaffNameIndex(nextText, "まな");
    manaNameIndex = nextRelativeIndex >= 0
      ? manaNameIndex + "まなこーぽれーしょん".length + nextRelativeIndex
      : -1;
  }
  const candidates = [
    ["mana_jobs", manaNameIndex],
    ["mana_narration", explicitStaffNameIndex(normalizedText, "しん")],
    ["mana_staff_dialogue", explicitStaffNameIndex(normalizedText, "ゆうき")],
    ...STAFF
      .filter(staff => !["mana_jobs", "mana_narration", "mana_staff_dialogue"].includes(staff.id))
      .map(staff => [staff.id, explicitStaffNameIndex(normalizedText, staff.name)]),
...
function parseStaffSpecificScriptCounts(targets, instruction) {
  ...
  const mentions = targets
    .map(staff => ({ staff, index: explicitStaffNameIndex(normalized, staff.name) }))
...
function assignTeamScriptCounts(targets, instruction, recognizedStaffCounts = new Map()) {
  const staffSpecificCounts = new Map(recognizedStaffCounts);
  parseStaffSpecificScriptCounts(targets, instruction)
    .forEach((count, staffId) => staffSpecificCounts.set(staffId, count));
...
const MANA_SHIN_YUKI_REVISION_PROMPT = `【しん・ゆうき共通・日本語と説明の最終品質確認】
完成台本を出力する前に一文ずつ確認し、一つでも満たさない場合は前後を含めて書き直す。確認結果は出力しない。
- 冒頭の主張または質問を一度で理解でき、その後の内容が同じ論点を深めていること。
- 結論だけでなく、なぜそう言えるのか、現場で何をするのか、その結果どう変わるのかまで説明すること。
- 質問には最初の一文で直接答え、理由と具体例を続けること。反論や疑問を出したら次の回答で解消し、説明しないまま理念へ飛ばないこと。
- 「人」「周り」「みんな」「それ」など曖昧な主語を避け、後輩スタッフ、同じ店舗のスタッフ、お客様など対象を明示すること。
- 時系列、原因、行動、結果の順序を守り、一文に目的、評価、理由を詰め込みすぎないこと。
- 抽象的な理念を使ったら、美容室で実際に取る行動を直後に一つ示すこと。
- 実在スタッフの体験として確認できない内容は断定せず、未確認の個人情報を作らないこと。
- 声に出して読み、意味を二度考えないと分からない文、質問と噛み合わない返答、不自然な助詞や動詞を残さないこと。

再発禁止:
- 「そこまで悩むまで気づけなかった」ではなく、「辞めたいと思うほど悩んでいることに気づけなかった」のように関係を明確にする。
- 「家賃や生活費を払った先」ではなく、「家賃や生活費を払ったあと、手元にいくら残るのか」のように生活の順序で話す。
- 「教える時間は取れなくないですか」のような二重否定を使わない。
- 「後輩の失敗を見る」「空気を見る」のような意味の広い動詞だけで済ませず、原因を一緒に考える、空気にも目を配るなど行動を示す。
...
const MANA_SHIN_YUKI_NARRATION_FLOW_PROMPT = `【しん・ゆうき共通・語りの自然なつなぎ】
- 「〜んだ」は強調が必要な場面だけに使う。「初めて見えたんだ」のように付け足さず、「初めて見えた」のように、そのまま言い切った方が自然なら短い常体を選ぶ。
- 「最初から完璧な計画は必要ない」のような結論を出す前に、「夢は、最初は漠然としていてもいい」のような受け止めを一文入れ、聞き手が結論へ自然についてこられる流れを作る。
- 前後の一文を続けて読み、二文目が急に結論へ飛んでいないか確認する。必要なら、聞き手の不安を受け止める短い一文を間に置く。`;

const MANA_SHIN_FINAL_REVIEW_PROMPT = `【しん専用・10本レビュー後の確定ルール】
この項目が、以前の共通口調ルールと矛盾する場合はこちらを優先する。
- 一人語りは、経験、転機、反省、信念を「変わった」「気づいた」「した」のように短く言い切る。ただし「〜なんだ」を機械的に付けない。
- 結論へ飛ばず、聞き手の迷いや不安を受け止める一文を置いてから主張へ進む。
- 「強い」「輝かせる」だけで終えず、誰が何にふさわしいのか、どんな立場である必要があるのかを明示する。
- 確定している会社の特徴は「会社でいたい」ではなく「会社です」と事実として述べ、本人の考えは「〜と思う」と区別する。
- 一つの主張、過去の考え、具体的な出来事、気づき、現在の考えを自然につなぎ、説明不足のまま理念で締めない。`;

const MANA_YUKI_FINAL_REVIEW_PROMPT = `【ゆうき専用・10本レビュー後の確定ルール】
この項目が、以前の共通口調・構成ルールと矛盾する場合はこちらを優先する。
- ゆうきの対談は、しんの一人語りのような断言調へ統一しない。瀬川社長は「〜だよね」「〜と思う」「〜かな」「〜やね」など、経験を振り返る自然な会話口調を使ってよい。美容師スタッフは自然な丁寧語で話す。
- 毎回カメラマンの質問から始めない。美容師スタッフの実体験から始め、カメラマンが別の見方を出し、社長が会社の考えを補足する流れも使う。
- カメラマンは「それなら」を機械的に付けず、直前の回答から動機、行動、結果、別の選択肢のいずれかを自然に聞く。
- 出来高制など別の働き方にも合う人がいることを認め、自社だけを正解にしない。本人が何を優先して選んだかを話す。
- 求人・働き方では、家賃や生活費を払ったあとに手元へいくら残るか、社会保険、結婚や将来、技術を学ぶ時期の収入不安など、生活の実感を入れる。未確認の個人史は作らない。
- 美容師スタッフは「不安→選んだ理由→働いて変わったこと」をつなぐ。瀬川社長は同じ説明を繰り返さず、会社が環境を整える理由や人を大切にする経営を話す。
- 当時の行動、起きた結果、現在の反省を順に話し、一般論の標語だけで終わらせない。
- 会社の制度や役割は確認済みの事実として正確に伝える。「勤務時間内に練習する時間を設ける」のように、助詞、漢字、句読点まで自然な日本語へ直す。
- 2人・3人、話し始める人物、質問の型、締め方を内容に合わせて変え、同じインタビュー構成へ固定しない。`;

const MANA_REVIEWED_SCRIPT_CORRECTIONS_PROMPT = `【10本の確定修正・完全反映版】
次は完成台本へ毎回そのまま転載する例文ではなく、日本語、質問順、説明の深さを判断する基準として使う。
- しんの店長テーマは、「店舗全体を良くすることはできない」「周りを輝かせる立場である必要がある」「それを本気で喜べる人が店長にふさわしいと思う」「仲間を輝かせられる人を育てる会社です」という明確さを基準にする。
- しんの夢テーマは、「やるべきことが初めて見えた」と簡潔に言い切り、「夢は最初は漠然としていてもいい」を挟んでから、完璧な計画は必要ないという結論へ進む。
- しんの退職テーマは、「自分に問いかける必要がある」と振り返り、退職後に外で人や会社と出会い学ぶことを肯定し、「困ったときに連絡を取り合える関係であり続けられる」「経営者の大切な仕事なんだと思う」と関係の継続へ着地する。
- ゆうきの人間関係テーマは、「これまでどうしていましたか。今考えると一番反省したことは何ですか」と時系列で聞き、「原因はこれだと決めつけていたんだよね」と振り返る。次は、問題から逃げたかったのか、早く解決したかったのかという動機を聞く。
- ゆうきの再入社テーマは、再び辞める可能性も認め、戻りたい理由、以前の悩み、会社が支えられることを確認し、最後はそのご縁を大切に迎え入れるという行動で締める。
- ゆうきのお客様テーマは、前回の悩みを覚えて次回来店時に尋ねる行動、表情を見る、仕上がりを一緒に確認する、時間や説明など小さな約束を守ることを信頼へつなげる。
- ゆうきの売上テーマは、「その売上をどうつくるか」を論点にする。物販等で単価だけを上げる例を出し、短期的なプラスは認めたうえで、再来店やスタッフ同士の協力まで見る。
- ゆうきの育成テーマは、「どういう行動が違うか」と聞く。短い声かけ、勤務時間内の練習時間、後輩の成長が店舗全体の信頼につながる流れを示す。
- ゆうきの生活テーマは、出来高制サロンにも興味があったことを認めつつ、収入、社会保険、生活の変化への不安を本人の比較として話す。「マナ」ではなく「マナコーポレーション」と正式名称を使う。
- ゆうきの働き方テーマは、美容師スタッフの「家賃や生活費を払ったあと、手元にいくら残るのか分からない」という実感から始めてよい。「働いてみてどうでしたか」と自然に聞き、技術、なりたい美容師像、長く働く未来を考えやすくなった変化へつなげる。`;
...
function applyManaShinYukiRevisionPrompt(settings) {
  ...
  ["mana_narration", "mana_staff_dialogue"].forEach(staffId => {
    ...
    if (!current.includes("【しん・ゆうき共通・語りの自然なつなぎ】")) {
      current = `${current}\n\n${MANA_SHIN_YUKI_NARRATION_FLOW_PROMPT}`;
      changed = true;
    }
    const finalReviewPrompt = staffId === "mana_narration"
      ? MANA_SHIN_FINAL_REVIEW_PROMPT
      : MANA_YUKI_FINAL_REVIEW_PROMPT;
    const finalReviewMarker = staffId === "mana_narration"
      ? "【しん専用・10本レビュー後の確定ルール】"
      : "【ゆうき専用・10本レビュー後の確定ルール】";
    if (!current.includes(finalReviewMarker)) {
      current = `${current}\n\n${finalReviewPrompt}`;
      changed = true;
    }
    if (!current.includes("【10本の確定修正・完全反映版】")) {
      current = `${current}\n\n${MANA_REVIEWED_SCRIPT_CORRECTIONS_PROMPT}`;
      changed = true;
    }
...
async function syncSharedStaffSettings() {
  try {
    ...
    if (Object.keys(shared).length) {
      ...
      const migratedManaQuality = applyManaShinYukiRevisionPrompt(staffSettings);
      const migrated = migratedAmane || migratedHiyori || migratedManaQuality;
      ...
      applyManaShinYukiRevisionPrompt(staffSettings);
...
let modalRecognizedStaffCounts = new Map();
...
function startModalRecognition() {
  ...
  rec.onresult = (e) => {
    ...
    const selectedStaff = activeModalStaffId
      ...
      : null;
    // 最新の認識結果で毎回入れ替え、途中の誤認識で別スタッフが残らないようにする。
    modalRecognizedStaffIds = new Set([
      ...(selectedStaff ? [selectedStaff.id] : []),
      ...recognizedStaff.map(staff => staff.id),
    ]);
    modalRecognizedTeamIds = new Set(recognizedTeams.map(team => team.id));
    modalRecognizedStaffCounts = parseStaffSpecificScriptCounts(
      recognizedStaff,
    ...
    const recognizedLabels = [...new Set([
      ...
      ...allRecognizedStaff
        ...
        .map(staff => {
          const count = modalRecognizedStaffCounts.get(staff.id);
          return `${staff.name}${count ? `${count}本` : ""}`;
        }),
    ])];
...
function openInstructionModal(staffId) {
  ...
  modalRecognizedStaffCounts = new Map();
...
async function handleInstructionSubmit(staffId, text, recognizedTargets = {}) {
  ...
  if (allExplicitTargets.length > 1 || effectiveCalledTeams.length) {
    ...
    const assignedTargets = assignTeamScriptCounts(
      allExplicitTargets,
      trimmed,
      new Map(recognizedTargets.staffCounts || [])
    );
...
function submitCurrentModalInstruction() {
  ...
  const recognizedTargets = {
    ...
    staffCounts: [...modalRecognizedStaffCounts.entries()],
  };
  stopModalRecognition(true);
...
const LIVER_DESK_STAFF = {
  liver_assistant: {
    ...
    note: "事務所側の報酬・収益条件は回答に含まれません／資料の原本は表示しません",
  ...
  liver_internal: {
    ...
    note: "社内用。事務所報酬を含む内容と、根拠資料の原本が表示されます",
  ...
  liver_agency: {
    ...
    note: "代理店向け。御社の適用額を金額で回答します（料率は非開示）／資料の原本は表示しません",
